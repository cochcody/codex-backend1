...
def apply_developer():
    ...
    try:
        ...
        print("Error loading dev apps:", repr(e))
    ...
    # Try JSONBin save, but DO NOT FAIL if it breaks
    try:
        save_dev_apps(apps)
    except Exception as e:
        print("Error saving dev apps to JSONBin:", repr(e))
        # Local save already succeeded — return success anyway
...
@require_admin
def admin_dev_apps():
    """Return all pending developer applications."""
    try:
        apps = load_dev_apps()
    except Exception as e:
...
def admin_dev_approve():
    """Approve a developer application and remove it from the list."""
    try:
        data = request.get_json(force=True)
    except Exception:
        return jsonify({"success": False, "message": "Invalid JSON"}), 400
...
def admin_dev_reject():
    """Reject a developer application and remove it from the list."""
    try:
        data = request.get_json(force=True)
    except Exception:
        return jsonify({"success": False, "message": "Invalid JSON"}), 400
...
@app.post("/api/admin/reset-dev-applications")
@require_admin
def reset_dev_applications():
    try:
        save_dev_apps([])  # this function already wraps it correctly
        return jsonify({"success": True})
    except Exception as e:
        print("Error resetting dev apps:", e)
