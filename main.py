...
def create_account():
    ...
    # ignore role from client; everyone starts as player
    role = "player"
...
@require_auth
def apply_developer():
    # player applies to become developer
    try:
        data = request.get_json(force=True)
    except Exception:
        return jsonify({"success": False, "message": "Invalid JSON"}), 400
    ...
    apps = load_json(DEV_APPS_PATH, DEFAULT_DEV_APPS)

    # prevent duplicate pending apps
    existing = next((a for a in apps["applications"] if a["email"] == email and a["status"] == "pending"), None)
    if existing:
...
def admin_dev_approve():
    ...
    app_entry = next((a for a in apps["applications"] if a["email"] == email), None)
    ...
    user["role"] = "developer"
    user["company"] = app_entry["company"]
...
def store():
    ...
    # Only show games that are NOT hidden
    visible_games = [
...
def purchase():
    # non-Stripe instant purchase (for testing / fallback)
    try:
        data = request.get_json(force=True)
    except Exception:
        return jsonify({"success": False, "message": "Invalid JSON"}), 400
...
def create_checkout_session():
    ...
    price = data.get("price")  # in dollars
