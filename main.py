...
def get_owned_games(email):
    ...
    return jsonify({"success": True, "owned_games": owned})
...
def admin_approve():
    ...
    accounts = load_json(ACCOUNTS_PATH, DEFAULT_ACCOUNTS)
    users = accounts.get("users", [])
    dev_user = next((u for u in users if u.get("email") == upload.get("developer_email")), None)

    dev_stripe = dev_user.get("stripe_account_id") if dev_user else None

    # add to games.json
    games = load_json(GAMES_PATH, DEFAULT_GAMES)
    game_list = games.setdefault("games", [])

    # avoid duplicates
    existing = next((g for g in game_list if g.get("id") == upload["id"]), None)
    if not existing:
        game_list.append({
            "id": upload["id"],
            "name": upload["name"],
            "price": float(upload["price"]),
            "folder": upload["folder"],
            "download": upload["download"],
            "developer_email": upload.get("developer_email"),
            "developer_stripe_account": dev_stripe,
            "description": upload.get("description", "No description provided."),
            "hidden": False,
            "featured": False
        })
        save_json(GAMES_PATH, games)
