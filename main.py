...
#  CONFIG
# ============================================================
FRONTEND_BASE_URL = "https://codexgames.com"  # adjust if needed
...
def admin_dev_reject():
    ...
    apps = load_dev_apps()
    apps = [a for a in apps if a.get("email") != email]
...
@app.post("/api/dev/stripe/create-account-link")
@require_developer
def dev_stripe_create_account_link():
    try:
        email = request.user["email"]
        ...
        # ⭐ PRODUCTION URLs (Render frontend)
        refresh_url = "https://frontend-xmfo.onrender.com/dashboard/dev.html"
        return_url  = "https://frontend-xmfo.onrender.com/dashboard/dev.html"
...
def purchase():
    ...
    if "owned" not in user or not isinstance(user["owned"], list):
        user["owned"] = []

    for g in games:
        if g not in user["owned"]:
            user["owned"].append(g)

    save_json(ACCOUNTS_PATH, accounts)

    return jsonify({"success": True, "owned": user["owned"]})

@app.get("/api/owned/<email>")
def get_owned_games(email):
    accounts = load_json(ACCOUNTS_PATH, DEFAULT_ACCOUNTS)
    users = accounts.get("users", [])
    user = next((u for u in users if u.get("email") == email), None)
    ...
    # Your launcher expects "owned", NOT "owned_games"
    owned = user.get("owned", [])

    return jsonify({"success": True, "owned": owned})
...
@app.post("/api/dev/stripe/create-account-link")
@require_developer
def dev_create_stripe_account_link():
    email = request.user["email"]
    ...
    if not acct_id:
        try:
            ...
            print("Stripe account error:", e)
            return jsonify({"success": False, "message": "Stripe account creation failed"}), 400

    try:
        refresh_url = urljoin(FRONTEND_BASE_URL, "/dashboard/dev.html?stripe=refresh")
        return_url = urljoin(FRONTEND_BASE_URL, "/dashboard/dev.html?stripe=return")
        ...
        return jsonify({"success": False, "message": "Stripe onboarding link failed"}), 400
#---------------
# Stripe payout
#----------------
@app.get("/api/dev/payouts")
@require_developer
def dev_payouts():
    email = request.user["email"]
    payouts = load_json(PAYOUTS_PATH, DEFAULT_PAYOUTS)
    dev_payouts = [
        p for p in payouts.get("payouts", [])
        if p.get("developer_email") == email
    ]
    return jsonify({"success": True, "payouts": dev_payouts})
...
def admin_get_uploads():
    uploads = load_json(UPLOADS_PATH, DEFAULT_UPLOADS)
    # uploads.json looks like: { "uploads": [ ... ] }
    return jsonify({ "uploads": uploads.get("uploads", []) })


# ============================================================
#  ADMIN ROUTE: APPROVE UPLOAD
# ============================================================
...
def admin_approve():
    ...
    # No developer lookup required anymore
    dev_email = upload.get("developer_email") or "unknown"
    dev_stripe = None
    ...
    games["games"].append({
        "id": upload["id"],
        "name": upload["name"],
        "price": float(upload.get("price", 0)),
        "developer_email": dev_email,
...
def admin_reject():
    ...
    upload = next((u for u in upload_list if u.get("id") == upload_id), None)
...
# Developer Analytics
#-----------------------
@app.get("/api/admin/revenue-by-game")
@require_admin
def admin_revenue_by_game():
    payouts = load_json(PAYOUTS_PATH, DEFAULT_PAYOUTS)
    by_game = {}
    for p in payouts.get("payouts", []):
        ...
        by_game[gid]["total_amount_cents"] += p.get("amount_total_cents", 0)
        by_game[gid]["platform_fee_cents"] += p.get("platform_fee_cents", 0)
        by_game[gid]["developer_amount_cents"] += p.get("developer_amount_cents", 0)
        by_game[gid]["sales"] += 1
    return jsonify({"games": list(by_game.values())})
